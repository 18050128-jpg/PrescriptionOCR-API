...
def create(extracted: dict, filename: str, image_path: str,
    ...
    """
    # Ensure medicines include a `remaining` field defaulting to `quantity`
    meds = []
    for m in extracted.get("medicines", []):
        med = dict(m)
        if "remaining" not in med or med.get("remaining") == "":
            med["remaining"] = med.get("quantity", "")
        meds.append(med)

    # Upsert medicines into global medicines DB for inventory tracking
    for m in extracted.get("medicines", []):
        try:
            medicine_repository.upsert_from_prescription_item(m)
        except Exception:
            # Không muốn block lưu prescription vì lỗi thêm vào kho
            pass
