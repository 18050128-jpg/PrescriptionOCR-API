...
class TestUploadOCR:
    ...
    def test_upload_returns_prescription_document(self, client, user_token):
        ...
        assert presc["medicines"][0]["remaining"] == presc["medicines"][0]["quantity"]
...
class TestUseMedicine:
    def test_use_medicine_decrements_remaining(self, client, prescription, user_token):
        original = prescription["medicines"][0]
        assert original["remaining"] == original["quantity"]

        r = client.put(
            f"/api/v1/prescriptions/{prescription['_id']}/medicines/0/use",
            headers=auth_headers(user_token),
        )
        assert r.status_code == 200
        updated = r.json()["prescription"]
        assert updated["medicines"][0]["remaining"] != original["remaining"]
        assert updated["medicines"][0]["remaining"].startswith("20")

    def test_use_medicine_requires_auth(self, client, prescription):
        r = client.put(f"/api/v1/prescriptions/{prescription['_id']}/medicines/0/use")
        assert r.status_code == 401
